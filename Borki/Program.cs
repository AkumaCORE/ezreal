    ...
    class Program
    {
        ...
        private static void Harass()
        {
                ...
                return;
            {
                ...
                if (HarassMenu["HarassR"].Cast<CheckBox>().CurrentValue && R.IsReady() && targetR.IsValidTarget(R.Range) && R.Handle.Ammo > HarassMenu["RocketHarass"].Cast<Slider>().CurrentValue)
                {
                    R.Cast(targetR);
                }
                if (ETarget == null) return;
                {
                    if (E.IsInRange(ETarget) && E.IsReady() && HarassMenu["HarassE"].Cast<CheckBox>().CurrentValue && ObjectManager.Player.IsInAutoAttackRange(ETarget))
                    {
                        E.Cast();
                    }
                }
        ...
        private static void LaneClear()
        {
            ...
            if (ClearMenu["ClearR"].Cast<CheckBox>().CurrentValue && minionQ != null && R.Handle.Ammo > ClearMenu["RocketClear"].Cast<Slider>().CurrentValue && R.IsReady())
                R.Cast(minionQ.ServerPosition);
            if (ClearMenu["ClearQ"].Cast<CheckBox>().CurrentValue && minionQ != null && Q.IsReady() && minionQ.IsValidTarget(Q.Range))
                Q.Cast(minionQ);
            if (ClearMenu["ClearE"].Cast<CheckBox>().CurrentValue && E.IsReady())
            {
                if (minions.Count() > 1)
                {
                    E.Cast();
